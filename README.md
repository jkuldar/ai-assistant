...
- Multi-turn conversation with sliding-window context management (last 20 messages)
...
- Nutrition queries: daily and weekly calorie/macro intake, meal breakdowns, date-range aggregation
...
4. The assistant remembers context within the conversation (up to 20 messages), so follow-up questions like "Can you tell me more about that?" work naturally.
...
1. **Conversation Layer** — Handles the chat interface, message history, input validation, and response rendering. The frontend maintains conversation history in-memory and in sessionStorage (survives page reloads) and sends it with each request. The backend enforces a sliding window of the last 20 messages to manage token usage.
...
| `get_nutrition_data` | Reads nutrition logs with calorie/macro totals for a date or date range | `date` (YYYY-MM-DD), `end_date` (optional YYYY-MM-DD) |
...
All tool parameters are validated before execution: dates are checked against YYYY-MM-DD format, `metric_type` is validated against the allowed enum, and `recipeId` is checked as a valid UUID. Invalid parameters return structured error messages that the model relays to the user.
...
- **Response format examples** — six per-query-type format examples (health metrics, meal plan overview, nutritional analysis, recipe details, progress & goals, general wellness) that demonstrate the expected structure and level of detail

### Conversation Memory

- Sliding window of the last 20 messages (10 user + 10 assistant turns)
- History is maintained client-side (in-memory + sessionStorage) and sent with each request
- The backend trims to the window size before passing to OpenAI
- History persists across page reloads via sessionStorage; cleared when the browser session ends
- No server-side persistence of chat history — conversations are ephemeral per browser session
...
- **Chat assistant**: `temperature=0.7, top_p=0.9` — conversational and natural while staying factual when referencing user data
